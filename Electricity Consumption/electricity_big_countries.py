...
def find_value(string_to_find):    # function that searches for an index
    i = 0
    for words in info[0]:
...
with open('factbook_updated.csv') as csv_file:
    csv_reader = csv.reader(csv_file, delimiter=',')    # reads and writes to a python variable
    ...
    graphed_countries = list_of_countries[0:4]
    print(graphed_countries)
    electricity_con = []
    electricity_prod = []

    for country in info_without_header[0:4]:     # Takes a group of countries and finds the values searched for
        consumption_index = float(country[find_value("Electricity - consumption(kWh)")])
        electricity_con.append(consumption_index / 10**12)       # compensates for the graphs value being Trillion
    ...
    plt.title('Electricity Consumption vs. Production\n(Big Countries)', fontsize = 20)
    plt.tick_params(labelsize = 20)
    plt.legend(loc = "upper left")
    plt.grid(alpha = 0.25)
    plt.savefig('electricity_big.jpg', dpi = 75)
    plt.show()
